...
private:
    ...
    this(RootWidget root) @safe pure nothrow
    {
        root_ = root;
        name_ = null;
    }
    ...
    this(ref WidgetAccess parent, const string name) @safe pure nothrow
    {
        name_ = name;
        root_ = parent.root_; 
    }
    ...
    string _fullName(const string childName) @safe pure nothrow const
    {
...
public:
    ...
    ///         and optional is false (default).
    ///         WidgetTypeException if the widget is not of type T.
    T get(T, Flag!"optional" optional = No.optional)(const string fullName) @safe pure nothrow
    {
...
protected:
    override void postInit() @safe pure nothrow
    {
        // Recursively generate addressToWidget_
        void widgetAddresses(Widget widget, string parentAddress) @safe pure nothrow
        {
...
package:
    ...
    void mouseLeftTree() @safe pure
    {
        void checkMouseOver(Widget widget)
        {
            if(rectangleIntersectsPoint(widget.layout.minBounds, widget.layout.maxBounds,
                                        guiSystem_.mousePosition))
            {
                widget.mouseLeft();
            }
            foreach(child; widget.children_)
    ...
    void checkMouseEnteredTree() @safe pure
    {
